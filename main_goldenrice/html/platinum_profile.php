<?php
	...
	if(isset($_COOKIE["platiUser"]))
	{
		$curr_User = $_COOKIE["platiUser"];
						
		$chek_Query = "SELECT * FROM platinum_user WHERE auth = '".$curr_User."' limit 1";
...
<html>
<head>
	<title> goldenrice/user/Platinum  </title>
</head>
<style>
	div#updateform
	{
		...
		<div >
			...
			<nav>
				<ul>
					...
					<li><a href="platinum_profile.php#cont_shop"> CONTACT </a></li>
					<li><a href="goldenrice_home.php#address"> ADDRESS </a></li>
					<li><a href="journal.php"> JOURNALS </a></li>
					<li><a href="platinum_profile.php#about_us"> ABOUT  </a></li>
					<?php
		...
		<span>
			<?php
					if(isset($_COOKIE["platiUser"]))
					{
						...
						$infoQuery = "SELECT * FROM platinum_user WHERE auth = '".$currentUser."' limit 1";
						...
						if ($run_infoQuery == true)
						{
							...
							{
								$a_id = $admin_data["pid"];
								$a_name = $admin_data["pname"];
								$a_email = $admin_data["pemail"];
								$a_phn_num = $admin_data["pphn_num"];
								$a_address = $admin_data["paddress"];
								$a_password = $admin_data["ppassword"];
								$a_joindate = $admin_data["pjoin_date"];
								$a_totalOrder = $admin_data["ptotal_order"];
								$a_point = $admin_data["ppoint"];
